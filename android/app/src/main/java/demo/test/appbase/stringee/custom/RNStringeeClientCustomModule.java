...
public class RNStringeeClientCustomModule extends ReactContextBaseJavaModule implements StringeeConnectionListener {

    private static final String TAG = "RNIC:RNStringeeClient";
    ...
    public RNStringeeClientCustomModule(ReactApplicationContext context) {
        super(context);
        mStringeeManager = StringeeManager.getInstance();
        mStringeeManager.setClient(null);
    }

    @NonNull
    @Override
    public String getName() {
        return "RNStringeeClientCustom";
    }
    ...
    private void sendEvent(ReactContext reactContext, String eventName, @Nullable WritableMap eventData) {
        while (reactContext == null);  // Busy wait.
        reactContext
    ...
    public void setNativeEvent(String event) {
        //Log.d(TAG, "Events will send: "+ event);
        jsEvents.add(event);
    }
