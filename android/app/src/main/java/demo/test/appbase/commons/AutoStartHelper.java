...
public class AutoStartHelper {
    ...
    private String PREF_KEY_APP_AUTO_START = "auto_start";
    ...
    public void getAutoStartPermission(Context context, Activity currentActivity) {
        prefUtil = PrefUtil.Init(context);

        String build_info = Build.BRAND.toLowerCase();
        switch (build_info) {
            case BRAND_ASUS:
                autoStartAsus(context, currentActivity);
                break;
            case BRAND_XIAOMI:
                autoStartXiaomi(context, currentActivity);
                break;
            case "redmi":
                autoStartXiaomi(context, currentActivity);
                break;
            case BRAND_LETV:
                autoStartLetv(context, currentActivity);
                break;
            case BRAND_HONOR:
                autoStartHonor(context, currentActivity);
                break;
            case BRAND_OPPO:
                autoStartOppo(context, currentActivity);
                break;
            case BRAND_VIVO:
                autoStartVivo(context, currentActivity);
                break;
            case BRAND_NOKIA:
                autoStartNokia(context, currentActivity);
                break;
    ...
    private void autoStartAsus(final Context context, Activity currentActivity) {



        if (isPackageExists(context, PACKAGE_ASUS_MAIN)) {

            showAlert(currentActivity, (dialog, which) -> {
                try {
                    prefUtil.save(PREF_KEY_APP_AUTO_START, true);
    ...
    private void showAlert(Activity currentActivity, DialogInterface.OnClickListener onClickListener) {
        AlertDialog.Builder alertDialog = new AlertDialog.Builder(currentActivity);
        alertDialog.setTitle("Allow AutoStart");
        alertDialog.setMessage("Please enable auto start in settings.");
    ...
    private void autoStartXiaomi(final Context context, Activity currentActivity) {
    ...
    private void autoStartLetv(final Context context, Activity currentActivity) {
        if (isPackageExists(context, PACKAGE_LETV_MAIN)) {
            showAlert(currentActivity, new DialogInterface.OnClickListener() {
                @Override
    ...
    private void autoStartHonor(final Context context, Activity currentActivity) {
    ...
    private void autoStartOppo(final Context context, Activity currentActivity) {
    ...
    private void autoStartVivo(final Context context, Activity currentActivity) {
    ...
    private void autoStartNokia(final Context context, Activity currentActivity) {
