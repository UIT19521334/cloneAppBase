...
import { getLabel, widthDevice, heightDevice } from '../../../utils/commons/commons';
...
class StringeeModule extends Component {
    ...
    constructor(props) {
        ...
        VoipPushNotification.addEventListener('notification', notification => {
            const callKitUUID = notification?.uuid || notification?._data?.uuid;
            const callSerial = notification?.serial || notification?._data?.serial;
            const callId = notification?.callId || notification?._data?.callId;
            console.log('Notification CallSerial: ' + JSON.stringify(notification || {}));
            console.log('Notification CallSerial: ', callKitUUID, callSerial, callId);
            // Neu call da duoc xu ly roi thi end callkit vua show
            if (this.callProcessed(callId, callSerial)) {
            ...
            // console.log('>>>>>>>>>>>>>>>>>>> call sync: ', this.state.syncCall);
    ...
    // ==================================== register Token For push Stringee ===================================
    registerTokenForStringee = () => {
        ...
        console.log('>>>>>>>>>>pushToken: ', this.state.pushToken);
    ...
    unregisterTokenForStringee = () => {
        ...
        this.refs?.stringeeClient?.unregisterPush(
            this.state.pushToken,
            (status, code, message) => {
                console.log('unregisterPush Stringee register VOIP: ' + message);
