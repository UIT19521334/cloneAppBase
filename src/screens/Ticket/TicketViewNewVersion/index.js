...
export class TicketView extends Component {
            ...
            if (parseInt(data.success) === 1) {
                ...
                this.setState({
                    ...
                    /** Added by
                         * @generator   :   Manh Le
                         * @date        :   2022-05-27 
                         * @description :   Process load information lead when API GetTicket not return info related_lead_name
                        */
                    if (!this.state?.ticket?.related_lead_name && this.state.ticket?.related_lead && !this.state.leadLoaded) {
                        this.updateRelatedLeadName(this.state.ticket?.related_lead);
                    }
     ...
     * @generator   :   Manh Le
     * @date        :   2022-05-27 
     * @description :   Func Process load information lead
    */
    updateRelatedLeadName(leadId) {
        const params = {
        ...
        this.setState({ loadingLead: true }, () => {
            Global.callAPI(null, params, (data) => {
                if (data?.success == 1) {
                    const ticket = this.state.ticket;
                    ticket.related_lead_name = data?.data?.label || "";

                    this.setState({ loadingLead: false, leadLoaded: true, ticket: ticket });

                }
                else {
                    this.setState({ loadingLead: false, leadLoaded: true });
                }
            }, (error) => {
                console.log('Fetch information Lead error: ', error);
                this.setState({ loadingLead: false, leadLoaded: true });
            })
        });
    }
