    ...
    renderTabOverView = () => {
        return (
            <TabContent style={{ backgroundColor: Colors.white.white3 }}>
                ...
                {/* 
                    * Add by Manh Le at 2022-05-27
                    * Description: rearrange the interface layout
                 */}
                <SectionView noPaddingHorizontal>
                    ...
                    <LineItemViewText
                        title={this.state.metaData?.field_list?.ticketcategories?.label || '---'}
                        value={Global.getEnumLabel('HelpDesk', 'ticketcategories', this.state.ticket?.ticketcategories || '')}
                    />
                    ...
                    <LineItemViewText
                        title={this.state.metaData?.field_list?.product_id?.label || '---'}
                        value={this.state.ticket?.product_name || ''}
                    />

                    <LineItemViewText
                        title={this.state.metaData?.field_list?.leadsource?.label || '---'}
                        value={Global.getEnumLabel('HelpDesk', 'leadsource', this.state.ticket?.leadsource || '')}
                    />

                    {
                        !this.state.ticket?.service_name ? null : (
                            <LineItemViewText
                                title={this.state.metaData?.field_list?.service_id?.label || '---'}
                                value={this.state.ticket?.service_name || ''}
                            />
                        )
                    }

                    <LineItemViewText
                        title={this.state.metaData?.field_list?.leadsource_level_2?.label || '---'}
                        value={Global.getEnumLabel('HelpDesk', 'leadsource_level_2', this.state.ticket?.leadsource_level_2 || '')}
                    />

                    <LineItemViewText
                        title={this.getFiledName('related_lead')?.label}
                        value={this.state.ticket?.related_lead_name || this.state.ticket?.related_lead || ''}
                        RenderRightButton={() => this.state.loadingLead ? <ActivityIndicator color={Colors.functional.primary} size={'small'} /> : null}
                        handleOnPress={() => this.state.ticket?.related_lead && this.props.navigation.navigate('LeadView', { lead: { id: this.state.ticket.related_lead }, prevScene: Global.getTicketViewLabel() })}
                    />
            ...
            return (
                <TabContent style={{ backgroundColor: Colors.white.white3 }}>
                    ...
                    <SectionView noPaddingHorizontal>
                        ...
                        {
                            this.state.ticket?.ticketcategories == "Bug" ? (
                                <LineItemViewText
                                    title={this.getFiledName('helpdesk_bug_type')?.label}
                                    value={Global.getEnumLabel('HelpDesk', 'helpdesk_bug_type', this.state.ticket?.helpdesk_bug_type || '')}
                                />
                            ) : null
                        }

                        {
                            this.state.ticket?.ticketcategories == "Bug" || this.state.ticket?.ticketcategories == "Request" ? (
                                <LineItemViewText
                                    title={this.getFiledName('helpdesk_feature_type')?.label}
                                    value={Global.getEnumLabel('HelpDesk', 'helpdesk_feature_type', this.state.ticket?.helpdesk_feature_type || '')}
                                />
                            ) : null
                        }

                        <LineItemViewText
                            title={this.getFiledName('helpdesk_customer_type')?.label}
                            value={Global.getEnumLabel('HelpDesk', 'helpdesk_customer_type', this.state.ticket?.helpdesk_customer_type || '')}
                        />

                        <LineItemViewText
                            title={this.getFiledName('ticketpriorities')?.label}
                            value={Global.getEnumLabel('HelpDesk', 'ticketpriorities', this.state.ticket?.ticketpriorities || '')}
                        />
                        ...
                        */}
                        {
                            !this.state.ticket?.service_name ? null : (
                                <LineItemViewText
                                    ...
                                    value={this.state.ticket?.service_name || this.state.ticket?.service_id || ''}
                                />
                            )
                        }
                        {/* End by Manh Le at 2022-05-27 */}
                        ...
                        {
                            Global.checkVersionCRMExist('7.1.0.20220415.1200') ? (
                                <>
                                    <LineItemViewText
                                    ...
                                    <LineItemViewText
                                        title={this.getFiledName('leadsource_level_2')?.label}
                                        value={Global.getEnumLabel('HelpDesk', 'leadsource_level_2', this.state.ticket?.leadsource_level_2 || '')}
                                    />
                        ...
                        <LineItemViewText
                            title={this.getFiledName('helpdesk_issue_category')?.label}
                            value={Global.getEnumLabel('HelpDesk', 'helpdesk_issue_category', this.state.ticket?.helpdesk_issue_category || '')}
                        />

                        {/* cancel_reason */}

                        {
                            this.state.ticket?.ticketstatus?.toUpperCase() == 'Cancel'.toUpperCase() ? (
                                <LineItemViewText
                                    title={this.getFiledName('cancel_reason')?.label}
                                    value={this.state.ticket?.cancel_reason || ''}
                                />
                    ...
                    <SectionView noPaddingHorizontal>
                        ...
                        <HeaderSectionView title={getLabel('ticket.title_more_information')} />

                        <LineItemViewText
                            title={this.getFiledName('helpdesk_device')?.label}
                            value={Global.getEnumLabel('HelpDesk', 'helpdesk_device', this.state.ticket?.helpdesk_device || '')}
                        />
                        <LineItemViewText
                            title={this.getFiledName('helpdesk_os')?.label}
                            value={Global.getEnumLabel('HelpDesk', 'helpdesk_os', this.state.ticket?.helpdesk_os || '')}
                        />
                        <LineItemViewText
                            title={this.getFiledName('helpdesk_application')?.label}
                            value={Global.getEnumLabel('HelpDesk', 'helpdesk_application', this.state.ticket?.helpdesk_application || '')}
                        />
                        <LineItemViewText
                            title={this.getFiledName('helpdesk_browser')?.label}
                            value={Global.getEnumLabel('HelpDesk', 'helpdesk_browser', this.state.ticket?.helpdesk_device || '')}
                        />
                        <LineItemViewText
                            title={this.getFiledName('browser_other')?.label}
                            value={this.state.ticket?.browser_other || ''}
                        />
                    ...
                    <SectionView noPaddingHorizontal>
                        ...
                        <HeaderSectionView title={getLabel('ticket.title_contact_information')} />
                        ...
                        */}
                        <LineItemViewText
                            title={this.getFiledName('related_lead')?.label}
                            value={this.state.ticket?.related_lead_name || this.state.ticket?.related_lead || ''}
                            RenderRightButton={() => this.state.loadingLead ? <ActivityIndicator size={'small'} color={Colors.functional.primary} /> : null}
                            handleOnPress={() => this.state.ticket?.related_lead && this.props.navigation.navigate('LeadView', { lead: { id: this.state.ticket.related_lead }, prevScene: Global.getTicketViewLabel() })}
                        />
                        {/* End by Manh Le at 2022-05-27 */}
                        ...
                        />
                        {
                            Global.checkVersionCRMExist('7.1.0.20220415.1200') ? (
                                <>
                                    <LineItemViewText
                                        title={this.getFiledName('contact_email')?.label}
                                        value={this.state.ticket?.contact_email || ''}
                                    />
                                    <LineItemViewText
                                        title={this.getFiledName('contact_mobile')?.label}
                                        value={this.state.ticket?.contact_mobile || ''}
                                    />
                                    <LineItemViewText
                                        title={this.getFiledName('helpdesk_related_emails')?.label}
                                        value={this.state.ticket?.helpdesk_related_emails || ''}
                                    />
                                </>
                            ) : null
                        }
                    ...
                    <SectionView noPaddingHorizontal>
...

                        {
                            this.state.ticket?.ticketcategories == "Bug" ? (
                                <>
                                    <LineItemViewText
                                        title={this.getFiledName('steps_to_reproduce')?.label}
                                        value={this.state.ticket?.steps_to_reproduce || ''}
                                    />
                                    <LineItemViewText
                                        title={this.getFiledName('expected_result')?.label}
                                        value={this.state.ticket?.expected_result || ''}
                                    />
                                    <LineItemViewText
                                        title={this.getFiledName('actual_result')?.label}
                                        value={this.state.ticket?.actual_result || ''}
                                    />
                    ...
                    <SectionView noPaddingHorizontal>
                        ...
                        <LineItemViewText
                            title={this.getFiledName('rating_note')?.label}
                            value={this.state.ticket?.rating_note || ''}
                        />
                        ...
                        Global.checkVersionCRMExist('7.1.0.20220415.1200') ? (
                            <>
                                ...
                                <SectionView noPaddingHorizontal>
                                    ...
                                    <HeaderSectionView title={getLabel('ticket.title_SLA_information')} />
                                    <LineItemViewText
                                        title={this.getFiledName('createdtime')?.label}
                                        value={this.state.ticket?.createdtime || ''}
                                    />
                                    <LineItemViewText
                                        title={this.getFiledName('related_cptickettaskcode')?.label}
                                        value={this.state.ticket?.related_cptickettaskcode || ''}
                                    />
                                    <LineItemViewText
                                        title={this.getFiledName('assignment_date')?.label}
                                        value={this.state.ticket?.assignment_date || ''}
                                    />

                                    <LineItemViewText
                                        title={this.getFiledName('process_start_date')?.label}
                                        value={this.state.ticket?.process_start_date || ''}
                                    />
                                    <LineItemViewText
                                        title={this.getFiledName('process_end_date')?.label}
                                        value={this.state.ticket?.process_end_date || ''}
                                    />
                                    <LineItemViewText
                                        title={this.getFiledName('sla_total_process_time')?.label}
                                        value={this.state.ticket?.sla_total_process_time || ''}
                                    />
                                    <LineItemViewText
                                        title={this.getFiledName('total_waiting_for_assignment_time')?.label}
                                        value={this.state.ticket?.total_waiting_for_assignment_time || ''}
                                    />
                                    <LineItemViewText
                                        title={this.getFiledName('total_process_time')?.label}
                                        value={this.state.ticket?.total_process_time || ''}
                                    />
                                    <LineItemViewText
                                        title={this.getFiledName('total_time')?.label}
                                        value={this.state.ticket?.total_time || ''}
                                    />
                                    <LineItemViewText
                                        title={this.getFiledName('over_sla')?.label}
                                        value={this.state.ticket?.over_sla == '1' ? getLabel('common.label_yes') : getLabel('common.label_no')}
                                    />
                                    <LineItemViewText
                                        title={this.getFiledName('helpdesk_over_sla_reason')?.label}
                                        value={this.state.ticket?.helpdesk_over_sla_reason || ''}
                                    />
                                    <LineItemViewText
                                        title={this.getFiledName('over_sla_note')?.label}
                                        value={this.state.ticket?.over_sla_note || ''}
                                    />
                            ...
                            (
                                <>
                                    ...
                                    <SectionView noPaddingHorizontal>
                                        ...
                                        />
                                        <LineItemViewText
                                            title={this.getFiledName('assgined_moment')?.label}
                                            value={this.state.ticket?.assgined_moment || ''}
                                        />

                                        <LineItemViewText
                                            title={this.getFiledName('finished_time')?.label}
                                            value={this.state.ticket?.finished_time || ''}
                                        />
                                        <LineItemViewText
                                            title={this.getFiledName('finished_moment')?.label}
                                            value={this.state.ticket?.finished_moment || ''}
                                        />
                                        <LineItemViewText
                                            title={this.getFiledName('closed_moment')?.label}
                                            value={this.state.ticket?.closed_moment || ''}
                                        />
                                        <LineItemViewText
                                            title={this.getFiledName('hours')?.label}
                                            value={this.state.ticket?.hours || ''}
                                        />
                                    </SectionView>
                        ...
                        Global.checkVersionCRMExist('7.1.0.20220415.1200') ? (
                            <>
                                ...
                                <SectionView noPaddingHorizontal>
                                    ...
                                    <HeaderSectionView title={getLabel('ticket.title_rating_information')} />
                                    <LineItemViewText
                                        title={this.getFiledName('is_send_survey')?.label}
                                        value={this.state.ticket?.is_send_survey == '1' ? getLabel('common.label_yes') : getLabel('common.label_no')}
                                    />
                                    <LineItemViewText
                                        title={this.getFiledName('helpdesk_survey_status')?.label}
                                        value={Global.getEnumLabel('HelpDesk', 'helpdesk_survey_status', this.state.ticket?.helpdesk_survey_status || '')}

                                    />
                                    <LineItemViewText
                                        title={this.getFiledName('satisfaction_score')?.label}
                                        value={this.state.ticket?.satisfaction_score || ''}
                                    />
                                    <LineItemViewText
                                        title={this.getFiledName('rating_description')?.label}
                                        value={this.state.ticket?.rating_description || ''}
                                    />
                    ...
                    <SectionView noPaddingHorizontal>
                        ...
                        <HeaderSectionView title={getLabel('ticket.title_management_information')} />
                        <LineItemViewText
