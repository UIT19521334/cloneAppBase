    ...
    if (date) {
      return (
        <View style={this.styles.day} testID={RESERVATION_DATE}>
          <Text  allowFontScaling={true}  allowFontScaling={true} style={[this.styles.dayNum, today]}>{date.getDate()}</Text>
          <Text  allowFontScaling={true}  allowFontScaling={true} style={[this.styles.dayText, today]}>{XDate.locales[XDate.defaultLocale].dayNamesShort[date.getDay()]}</Text>
        </View>
      );
    } else {
      return (
